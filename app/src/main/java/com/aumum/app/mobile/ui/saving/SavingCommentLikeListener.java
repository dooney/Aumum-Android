 ...
 */
public class SavingCommentLikeListener implements LikeTextView.OnLikeListener {
    ...
    public SavingCommentLikeListener(SavingComment comment) {
    ...
    public void onUnLike(LikeTextView view) {
        ...
        task = new SafeAsyncTask<Boolean>() {
            public Boolean call() throws Exception {
                ...
                restService.removeSavingCommentLike(comment.getObjectId(), currentUser.getObjectId());
                return true;
            }
    ...
    public void onLike(final LikeTextView view) {
        ...
        task = new SafeAsyncTask<Boolean>() {
            public Boolean call() throws Exception {
                User currentUser = userStore.getCurrentUser();
                restService.addSavingCommentLike(comment.getObjectId(), currentUser.getObjectId());
    ...
    private void sendLikeMessage(Context context, User currentUser) throws Exception {
        if (!comment.getUserId().equals(currentUser.getObjectId())) {
            String title = context.getString(R.string.label_like_saving_comment_message,
                    currentUser.getScreenName());
            CmdMessage cmdMessage = new CmdMessage(CmdMessage.Type.MOMENT_COMMENT_LIKE,
