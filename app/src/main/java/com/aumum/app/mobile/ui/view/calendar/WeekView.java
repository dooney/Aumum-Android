package com.aumum.app.mobile.ui.view.calendar;
...
public class WeekView extends View {
    ...
    private int mFirstVisibleHour = 5;
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mTimeTextSize = a.getDimensionPixelSize(R.styleable.WeekView_timeTextSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mTimeTextSize, context.getResources().getDisplayMetrics()));
    ...
    private void init() {
        ...
        Rect rect = new Rect();
        mTimeTextPaint.getTextBounds("上午12点", 0, "上午12点".length(), rect);
        mTimeTextWidth = mTimeTextPaint.measureText("上午12点");
        ...
        mHeaderTextPaint.getTextBounds("12月12号", 0, "12月12号".length(), rect);
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            String time = getDateTimeInterpreter().interpretTime((i + mFirstVisibleHour) % 24);
            if (time == null)
        ...
        for (int i = 0; i < maxRowCount; i++) {
            ...
            for (List<EventRect> column : columns) {
                if (column.size() >= i+1) {
                    ...
                    int startHourOfDay = eventRect.event.getStartTime().get(Calendar.HOUR_OF_DAY);
                    int endHourOfDay = eventRect.event.getEndTime().get(Calendar.HOUR_OF_DAY);
                    eventRect.top = ((startHourOfDay + 24 - mFirstVisibleHour) % 24 ) * 60 + eventRect.event.getStartTime().get(Calendar.MINUTE);
                    eventRect.bottom = ((endHourOfDay + 24 - mFirstVisibleHour) % 24 ) * 60 + eventRect.event.getEndTime().get(Calendar.MINUTE);
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    return String.format("%d月%02d号", date.get(Calendar.MONTH) + 1, date.get(Calendar.DAY_OF_MONTH));
                }
